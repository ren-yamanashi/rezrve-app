...
export function useSchedule() {
	...
	const [rsv11,setRsv11] = useRecoilState(rsvState2);
	...
	async function loadSchedule11() {
		setError11(false);
		const snapshot = await getDocs(query(baseQuery(today),where("time","==",11)));
		if(snapshot.empty) {
			setError11(true)
		}
		const gotFreeList = snapshot.docs.map((doc) => {
	...
	return {loadSchedule,rsv,error10,loadSchedule10,rsv10,error11,loadSchedule11,rsv11}
}