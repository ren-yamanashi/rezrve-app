...
export function useSchedule() {
	...
	return {loadSchedule,rsv,error10,loadSchedule10,rsv10,baseQuery}
}
export function useSchedule11 () {
	const {baseQuery} = useSchedule();
	const {user} = useAuth()
	...
	async function loadSchedule11(newDate) {
		...
		const snapshot = await getDocs(query(
			collection(db,"FreeSpace"),
			where("senderUid","==",user.uid),
			where("date","==",timestamp(newDate)),where("time","==",11)));
		if(snapshot.empty) {
			setError11(true)
		}
		const gotFreeList = snapshot.docs.map((doc) => {
	...
	return {loadSchedule11,rsv11,error11}
}