...
export function useFreeSpace_newValue() {
	...
	return {freeSpaces,error,setError,loadFreeSpace_newValue}
}
/**=================================
...
export function useSelectTeacher() {
	...
	async function loadSelectTeacher(teacher,date) {
		setErr(false);
		const snapshot = await getDocs(query(baseQuery(),where("date","==",timestamp(date)),where("teacher","==",teacher),orderBy("time","asc")))
	}
}
/**================================
