...
export const useGetReserves = () => {
	...
	const getReserves = async({e,date,time,person,id,email,phoneNumber,reserver,companyId}:GetReserveProps) => {
		e.preventDefault();
		try {
			const snapshot = await getDocs(query(
				baseQuery(companyId),
...
export const useChancelRsv = () => {
	const {showErrorMessage,showSuccessMessage} = useAlert()
	const router = useRouter();
    const chancelRsv = async(e: any,id:string,loads) =>{
		e.stopPropagation();
		try {
			await updateDoc(doc(db, "FreeSpace", id), {
				reserved: false,
