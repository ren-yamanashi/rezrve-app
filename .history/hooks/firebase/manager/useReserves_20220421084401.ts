...
export const useSchedule = () => {
	...
	const rsvQuery = async (date,teacher) => {
		return query(
			collection(db,"FreeSpace"),
	...
	const rsvLoad = (snapshot:QuerySnapshot<DocumentData>) => {
		const gotFreeList = snapshot.docs.map((item) => {
			const rsv = item.data() as FreeList;
			rsv.id = item.id;
			return rsv
		})
	...
	const getReserveList = async(date,teacher) => {
		const snapshot =  await getDocs(await rsvQuery(date,teacher));
		rsvLoad(snapshot);
	};
	// create base hooks
	...
	return {rsvArr,loadScheduleAll,getReserveList}
}