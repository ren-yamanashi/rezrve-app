...
export const rsvState_Object = atom({
	key:"rsv",
	default:{normal:initialRsv,today:initialRsv,week:initialRsv},
})
...
export const useReserves = () => {
	const [reserveData,setReserveData] = useRecoilState(rsvState_Object)
	...
	const baseLoading = (snapshot:QuerySnapshot<DocumentData>,number?) => {
		...
		number == 1 && setReserveData({...reserveData,normal:gotRsv});
		number == 2 && setReserveData({...reserveData,today:gotRsv});
		number == 3 && setReserveData({...reserveData,week:gotRsv});
		
	};
	return {baseQuery,baseLoading,rsv,rsv_today,rsv_week,reserveData}
}
/**======== all =======*/
export const useReservesAll = () => {
	const {baseQuery,baseLoading,reserveData} = useReserves()
...
export const useReserves_Today = () => {
	...
	return {error,loadRsv,reserveData}
}
/**====== onesWeek ========*/
...
export const useSchedule = () => {
    ...
    const loadScheduleAll = async(newDate) => {
		const loadSchedule = async() => {
			const snapshot = await getDocs(( await baseQuery(newDate)));
