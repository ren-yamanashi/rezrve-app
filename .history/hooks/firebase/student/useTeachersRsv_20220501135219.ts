...
export const useTeachersRsv_schedule = () => {
  ...
  const [reserve,setReserve] = React.useState({rsv1:[],rsv2:[],rsv3:[],rsv4:[],rsv5:[],rsv6:[],rsv7:[]})
  const rsvArr = [reserve.rsv1,reserve.rsv2,reserve.rsv3,reserve.rsv4,reserve.rsv5,reserve.rsv6,reserve.rsv7]
  ...
  const baseLoading = (snapshot: QuerySnapshot<DocumentData>, number) => {
    ...
    number ==1 && setReserve({...reserve,rsv1:gotRsv})
    number ==2 && setReserve({...reserve,rsv2:gotRsv})
    number ==3 && setReserve({...reserve,rsv3:gotRsv})
    number ==4 && setReserve({...reserve,rsv4:gotRsv})
    number ==5 && setReserve({...reserve,rsv5:gotRsv})
    number ==6 && setReserve({...reserve,rsv6:gotRsv})
    number ==7 && setReserve({...reserve,rsv7:gotRsv})
