...
const SelectTeacherModal = (props) => {
  const { loadSelectTeacher, loadFsError } = useSelectTeacher();
  const { newDateTime } = useDate();
  const { handleClose7, onOpen7 } = useHandle();
  const { loadRsvTeacher, loadRsvError } = useReserves_teacher();
                  ...
                  >
                    <PrimaryBtn
                      ...
                      click={() => {
                        props?.load == 1 &&
                          router
                            .push(`/reserve/manager/teachers/${user.id}`)
                            .then(() => router.reload());
                        props?.load == 2 &&
                          router
                            .push(`/reserve/teachers/${user.id}`)
                            .then(() => router.reload());
