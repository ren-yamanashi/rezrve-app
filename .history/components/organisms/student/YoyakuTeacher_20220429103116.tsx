...
// import my File
import { useStaffList_queryId } from "../../../hooks/firebase/user/useUserList";
...
// 担当者から探す
const YoyakuTeacher = () => {
  const { usersList, loadStaffList_query } = useStaffList_queryId();
  ...
  React.useEffect(() => {
    ...
    loadStaffList_query(query?.id);
  }, [process.browser, query?.id]);
  return (
    <>
      <React.Fragment>
          ...
          <Media at="sm">
            ...
            >
              {!usersList || usersList.length == 0 ? (
                <AlertComponent>予約可能な講師はいません</AlertComponent>
              ) : (
                usersList.map((index) => (
                  <>
                    <Box mb={3} display="flex" mx={2}>
