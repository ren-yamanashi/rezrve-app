...
import { useReserves_Today, useReserves_week } from "../../hooks/useReserves";
...
export default function Reserves() {
  ...
  const { user } = useAuth();
  const { rsv } = useReserves_week();
  const { error } = useReserves_week();
  ...
  useEffect(() => {
    ...
    async function loadReserves() {
      console.log(rsv.map((i) => dayjs(i.date.toDate())));
      const q = query(
