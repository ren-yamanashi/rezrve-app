...
export default function Reserves() {
   ...
   * 完了・未完了の設定
   *=========*/
  const UpdateReserve = async (object) => {
    const db = getFirestore();
    const Reserve = doc(db, "ReserveList", object.id);
    await updateDoc(Reserve, {
      completed: !object.reserve.completed,
    });
  };
  return (
    <React.Fragment>
        ...
        <TableBody>
          {Reserves.map((rsv) => (
            <TableRow key={rsv.id}>
              ...
              <TableCell>
                <Checkbox checked={rsv.completed} onClick={UpdateReserve} />
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
