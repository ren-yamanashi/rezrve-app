...
const SelectDayAll = () => {
  ...
  const { user } = useAuth();
  const { changeDateAndSetDateValue, changeDateValue, newDateTime, dateValue } =
    useDate();
        ...
        {loading == true ? (
          ...
          <>
            <DateRangePicker
              value={dateValue}
              changeDate={(newValue) => {
                changeDateAndSetDateValue(newValue);
              }}
            />
            <GetRsv_OK_Cancel />
            <Box>
                              ...
                              rsv10.map((item) => {
                                uu.userName == item.teacher &&
                                  setRsvData({
                                    ...
                                    rsvTime: item.time,
                                    date: `${dayjs(item.date.toDate()).format(
                                      "YYYY/MM/DD "
                                    )} ${item.time}:00~`,
                                  });
                                uu.userName == item.teacher &&
                        ...
                        <TableCellComponent8
                          ...
                          clickEvent={() =>
                            ...
                            rsv11.map((item) => {
                              ...
                              uu.userName == item.teacher && item.student !== ""
                                ? handleOpen4()
                                : handleOpen();
                            })
                          }
