...
const YoyakuListToday = () => {
  ...
  React.useEffect(() => {
    ...
    loadUser_query(user.uid).then(() => loadRsv(user_query?.companyId));
  }, [process.browser, user, user_query]);
  return (
    <React.Fragment>
      <Box>
        <CardContent
        ...
        <RsvModal
          ...
          chancelRsv={(e) => {
            chancelRsv(e, rsvData.id, handleClose4()).then(() => {
              loadRsv(user_query.companyId);
              loadShift(newDateTime, user_query?.companyId);
            });
          }}
        />
      </Box>
