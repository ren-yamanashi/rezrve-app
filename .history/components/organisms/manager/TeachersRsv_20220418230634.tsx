...
const TeachersRsv = () => {
  ...
  const dateArr = [
    ...
    { date: dayArr.dy, dateX: dayArrX.dy, dateY: dayArrY.dy, number: 1 },
    { date: dayArr.dy2, dateX: dayArrX.dy2, dateY: dayArrY.dy2, number: 2 },
    { date: dayArr.dy3, dateX: dayArrX.dy3, dateY: dayArrY.dy3, number: 3 },
    { date: dayArr.dy4, dateX: dayArrX.dy4, dateY: dayArrY.dy4, number: 4 },
    { date: dayArr.dy5, dateX: dayArrX.dy5, dateY: dayArrY.dy5, number: 5 },
    { date: dayArr.dy6, dateX: dayArrX.dy6, dateY: dayArrY.dy6, number: 6 },
  ];
  const time_arr = [10, 11, 12, 13, 14, 15, 16, 17, 18];
  dateArr.map((item) => {
    item.date.setDate(item.date.getDate() + item.number);
    item.date.setHours(12, 0, 0);
    item.dateX.setDate(item.dateX.getDate() + 7 + item.number);
    item.dateX.setHours(12, 0, 0);
    item.dateY.setDate(item.dateY.getDate() - 7 + item.number);
    item.dateY.setHours(12, 0, 0);
  });
  console.log(dayArrX.today);
  console.log(dayArrY.today);
              ...
              <Media greaterThan="sm">
                ...
                >
                  <IconButton
                    onClick={() => {
                      changeDateValue(
                        dayArrY.today.setDate(dayArrY.today.getDate() - 7)
                      );
                      loadRsvScheduleAll_Y(
                        user_query && user_query.userName,
                        dayArrY.today,
