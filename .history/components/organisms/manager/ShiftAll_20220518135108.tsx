...
//　提出シフト一覧
const ShiftsAll: React.FC<{ shifts: reserveProps[]; times: timeProps }> = ({
  shifts,
  times,
}) => {
  ...
  //　日付に該当するシフトのみで新しい配列を作る
  const shiftArr = [];
  shifts.map((shift) => {
    format(newDateTime, "yyyy-MM-dd") ==
      format(new Date(shift.date), "yyyy-MM-dd") && shiftArr.push(shift);
  });
  return (
    <React.Fragment>
      {loading == true ? (
        ...
        <>
          <Box ml={3}>
            ...
            <Box>
              <IconButton
                onClick={() => router.push(`/${user?.displayName}/staffs`)}
              >
