...
const SelectDayAll = () => {
  ...
  const { usersList, loadTeachersList } = useTeacherList();
  const { getReserves } = useGetReserves();
  const { loadScheduleAll, rsvArr, getReserveList, reserve } = useSchedule();
  const {
    selectRsv,
    selectTeacher,
    handleChange,
  ...
  useEffect(() => {
    ...
    usersList.map((uu) => getReserveList(newDateTime, uu.userName));
          ...
          <>
            <DateRangePicker
              ...
              changeDate={(newValue) => {
                ...
                getReserveList(
                  chgDate(newValue),
                  usersList.map((uu) => getReserveList(newDateTime, uu))
                );
              }}
            />
            <GetRsv_OK_Cancel />
