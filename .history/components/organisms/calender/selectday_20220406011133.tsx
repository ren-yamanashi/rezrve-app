...
import GetRsv_OK_Cancel from "../../templates/andMore.../GetRsv_OKCancel2";
...
export default function SelectDayAll() {
  ...
  const { loadRsv, loadError } = useReserves_Today();
  const { usersList, loadUserError } = useUserList();
  const { chancelRsv, deleteShiftError } = useDeleteShift();
  ...
  const { createShift, cre } = useCreateShift();
  ...
  return (
    <>
      <React.Fragment>
        ...
        {deleteShiftError && deleteShiftError == true && (
          <AlertComponent>シフトの削除に失敗しました</AlertComponent>
        )}
        <CreateShiftModal
