...
import GetRsv_OK_Cancel from "../../templates/andMore.../GetRsv_OK_Cancel3";
...
export default function SelectDayAll() {
  ...
  const { createShift, createError } = useCreateShift();
  const { chancelRsv, loadRsvError } = useChancelRsv();
  const { usersList } = useTeacherList();
  const { studentsList, loadSearchStudentsList, loadUserError } =
    useStudentsList();
  ...
  useEffect(() => {
    ...
    loadScheduleAll(newDateTime).then(() => setLoading(false));
          ...
          <>
            <DateRangePicker
              ...
              changeDate={async (newValue) => {
                ...
                loadScheduleAll(newDate).then(() => setLoading(false));
              }}
            />
            <GetRsv_OK_Cancel />
            <Box>
