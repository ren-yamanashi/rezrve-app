...
import { RsvModal } from "../modal/RsvModal";
...
export default function SelectDay() {
  ...
  let d = day.getDate();
  let date = `${y}/${m}/${d}`;
  const onClickRsv = useCallback(
    (id: string) => {
      onSelectRsv({ id, freeLists, onOpen });
      console.log(id);
    },
    [freeLists, onSelectRsv, onOpen]
  );
  /**========
   * Firebaseからデータを取得
   *========*/
  ...
  return (
    <>
      ...
      <React.Fragment>
              ...
              <List>
                {freeLists10.map((item) => (
                  <ListItem key={item.id}>
                    <Button variant="contained" fullWidth onClick={onClickRsv}>
                      <ListItemText
        ...
        <RsvModal rsv={selectedRsv} isOpen={isOpen} onClose={onClose} />
      </React.Fragment>
    </>
  );
}
